...
export const metadata: Metadata = {
    ...
    keywords: 'recruitment software for startups, HR automation tools, recruitment agency software, enterprise hiring platform, AI hiring for small business',
};
...
const useCases = [
    {
        ...
        stat: '10x faster than manual screening',
    },
    {
        emoji: '🏢',
        title: 'Recruitment Agencies',
        subtitle: 'Manage multiple clients and roles at once',
        description: 'Agencies running 10+ active roles need a system that scales. Recruit Flow\'s Enterprise plan gives you 10 simultaneous job postings, a global candidate CRM, and bulk PDF export for client presentations.',
        ...
        stat: 'Manage 10 roles simultaneously',
    },
    {
        emoji: '👥',
        ...
        stat: 'Reduce time-to-hire by 60%',
    },
    {
        emoji: '🎓',
        ...
        stat: 'Process 200+ applications per role',
    },
    {
        emoji: '💼',
        ...
        stat: 'Pro plan from NPR 299/month',
    },
    {
        emoji: '🌍',
        ...
        stat: 'Hire from anywhere in the world',
    },
];
        ...
        <div className="min-h-screen bg-white font-sans">
            ...
            <section className="py-24 px-6">
                <div className="max-w-6xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {useCases.map((uc, i) => (
                        <article key={i} className="bg-white border border-slate-100 rounded-sm p-8 hover:shadow-lg transition-all hover:-translate-y-1 duration-300">
                            <div className="text-4xl mb-4">{uc.emoji}</div>
                            <h2 className="text-xl font-black text-slate-900 mb-1">{uc.title}</h2>
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">{uc.subtitle}</p>
                            <p className="text-sm text-slate-600 leading-relaxed mb-5">{uc.description}</p>
                            <ul className="space-y-2 mb-6">
                                {uc.points.map((p, j) => (
                                    <li key={j} className="flex items-start gap-2 text-sm text-slate-600">
                                        <span className="size-1.5 bg-emerald-500 rounded-full shrink-0 mt-1.5" />
                                        {p}
                                    </li>
                                ))}
                            </ul>
                            <div className="pt-4 border-t border-slate-100">
                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{uc.stat}</span>
                            </div>
                        </article>
                    ))}
                </div>
