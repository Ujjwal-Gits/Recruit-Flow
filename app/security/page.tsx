...
export const metadata: Metadata = {
    ...
    keywords: 'recruitment software security, HR data privacy, secure ATS platform, GDPR recruitment software, candidate data protection',
};

const securityFeatures = [
    { icon: '🔐', title: 'JWT Authentication', desc: 'Every session uses a signed JSON Web Token verified by Supabase Auth. Tokens expire automatically and are refreshed securely via HTTP-only cookies.' },
    { icon: '🛡️', title: 'Row-Level Security', desc: 'Every database table has RLS policies enforced at the PostgreSQL level. Recruiters can only access their own jobs and candidates — no cross-account data leakage is possible.' },
    { icon: '📧', title: 'OTP Verification', desc: 'Registration, password reset, and profile changes all require a 6-digit OTP sent to your email. Codes expire in 2 minutes and are invalidated after 5 failed attempts.' },
    { icon: '⚡', title: 'Rate Limiting', desc: 'All authentication endpoints are rate-limited by IP address. Login allows 5 attempts per 5 minutes. Registration allows 3 per 10 minutes. This prevents brute-force attacks.' },
    { icon: '🔒', title: 'Secure File Storage', desc: 'Resumes are stored in Supabase Storage with randomized filenames. No personally identifiable information is included in storage paths.' },
    { icon: '🌐', title: 'HTTPS Only', desc: 'All data in transit is encrypted via TLS. Cookies are set with SameSite=Lax and Secure flags in production to prevent CSRF and interception.' },
    { icon: '👁️', title: 'Admin Access Control', desc: 'Admin routes are protected at both the middleware and API level. Only users with owner, manager, support, or admin roles can access the admin panel.' },
    { icon: '🗑️', title: 'Data Isolation', desc: 'Each recruiter\'s data is completely isolated. You cannot access another recruiter\'s jobs, candidates, or profile — even if you know their user ID.' },
];
            ...
            <section className="py-24 px-6">
                <div className="max-w-5xl mx-auto">
                    <div className="text-center mb-16">
                        ...
                        <p className="text-slate-500 max-w-xl mx-auto">We don't bolt on security as an afterthought. Every feature is designed with data protection in mind from the start.</p>
                    </div>
                    <div className="grid md:grid-cols-2 gap-6">
                        {securityFeatures.map((f, i) => (
                            <div key={i} className="bg-white border border-slate-100 rounded-sm p-6 flex gap-4">
